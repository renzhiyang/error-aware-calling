...
def get_sequence(full_read_sequence:str, position:int, is_forward:bool,
                        insert_len:int , delete_len: int, is_indel:bool, config:DictConfig):
    """
...
def label_unphased_read(chrom:str, read:pysam.AlignedSegment, 
    ...
    ref_seq = ref_seq[read.reference_start: read.reference_start + 500] #将reference截断，减少后续索引的计算量
    ...
    for cigar_tuple in read.cigar:
        ...
        if cigar_op == 0: # match pr mismatch
            for i in range(length):
                if ref_pos_clip + i >= len(ref_seq) or read_pos + i >= len(full_read_sequence): 
                    continue  # Avoid index out of range
                if not is_pos_in_confident_region(ref_pos_origin + i, confident_regions):
                    continue
                ...
                if ref_base != read_base and ref_pos_origin + i + 1  not in variants: 
                    ...
                    #                                      is_indel=False, 
                    #                                      config=config)
                    sequence_around = get_sequence(full_read_sequence=ref_seq,
                    ...
                    print_label(chrom=chrom, type='Negative', read_strand=read_strand,
                                position=ref_pos_origin+i+1, label=ref_base, 
            ...
            if not is_germline:
                ...
                sequence_around = get_sequence(full_read_sequence=ref_seq, 
                                               ...
                                               config=config)
                print_label(chrom=chrom, type='Negative', read_strand=read_strand,
                            position=ref_pos_origin, label=ref_base, 
            ...
            if not is_germline:
                ...
                sequence_around = get_sequence(full_read_sequence=ref_seq,
                                               ...
                                               config=config)
                print_label(chrom=chrom, type='Negative', read_strand=read_strand,
                            position=ref_pos_origin, label=ref_base + deleted_bases + deleted_next_base, 
        ...
        if cigar_op == 0: # match or mismatch
            ...
            for i in range(length - 1): # match的以后一个是indel的开端，因此这个位点放到indel里去处理
                ...
                if not is_germline and read_base != label:
                    ...
                    print_label(chrom=chrom, type='Negative', read_strand=read_strand,
                                position=ref_pos_origin+i+1, label=label, 
                                read_base=read_base, ref_base=ref_base, 
                ...
                if is_germline:
                    ...
                    label = variants[ref_pos_origin + i + 1]['haplotype'][haplotype_index] # variants are 1-based
            ...
            inserted_bases = full_read_sequence[read_pos: read_pos + length]
            read_base = full_read_sequence[read_pos - 1]
            ref_base = ref_seq[ref_pos_clip - 1] # insertion是以插入bases的前一个位点为基准
            is_germline = True if ref_pos_origin in variants else False
            alts = None
            ...
            if is_germline:
                ...
                if is_phased_variant:
                    ...
                    if combined == label:
                        print_label(chrom=chrom, type='Positive', read_strand=read_strand,
                                    position=ref_pos_origin, label=label, 
                                    read_base=combined, ref_base=ref_base, 
            ...
            if is_germline:
                ...
                if is_phased_variant:
                    ...
                    if len(deleted_bases) == len(another_hap) - len(label):
                        print_label(chrom=chrom, type='Positive', read_strand=read_strand,
                                    position=ref_pos_origin, label=label + deleted_next_base, 
                                    read_base=read_base + len(deleted_bases)*'-' + deleted_next_base, 
                                    ref_base=ref_base+deleted_bases, 
