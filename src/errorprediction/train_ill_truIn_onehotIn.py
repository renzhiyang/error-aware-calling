...
def train(model, train_loader, test_loader, criterion_1, criterion_2, optimizer, config):
    save_interval = 2
        ...
        for inputs, labels_1, labels_2 in test_loader:
            ...
            for i in range(next_base.shape[0]):
                ...
                if next_base[i] == true_next_base[i] and insertion[i] == true_insertion[i]:
                    correct += 1
    print(f'# of correct next base: {next_base_correct}, # of correct insertion: {insertion_correct}')
...
def train_only_first(model, train_loader, test_loader, criterion, optimizer, config):
    ...
    for epoch in range(epochs):
        ...
        val_loss, accuracy = test_only_first(model, test_loader, criterion,)
        writer.add_scalar('Loss/test', val_loss, epoch)
        writer.add_scalar('Accuracy/test', accuracy, epoch)
        writer.flush()
        print(f'Time:{datetime.now()}, Epoch {epoch+1}/{epochs}, Training loss: {epoch_loss:.4f}, Accuracy: {accuracy:.4f}', flush=True)
...
def main(config: DictConfig) -> None:
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=config.training.learning_rate, weight_decay=1e-3) # type: ignore
