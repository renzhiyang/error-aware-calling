...
def get_candidates(args):
    ...
    regions = [f"{args.ctg_name}:{args.ctg_start}-{args.ctg_end}"] if args.ctg_name else []
    ...
    for zero_based_position in positions:
        if args.ctg_start <= zero_based_position + 1 <= args.ctg_end:
            ...
            depth = sum(x[1] for x in base_count) - pileup[zero_based_position]["I"] - pileup[zero_based_position]["D"]
            if depth < args.min_coverage:
                continue
            base_count.sort(key=lambda x: -x[1])
            if base_count[0][0] != ref_base or (float(base_count[1][1]) / depth) >= args.min_allele_freq:
                print(f"{args.ctg_name} {zero_based_position + 1} {ref_base} {depth} " + " ".join([f"{x[0]} {x[1]}" for x in base_count]),
                      flush=True)
...
def main():
    ...
    parser.add_argument('--bam_fn', type=str, help='input alignment bam file', 
                        default="", required=True)
    parser.add_argument('--samtools', type=str, help="Path to the samtools", 
                        default="samtools")
    parser.add_argument('--min_coverage', type=int, help="minimum coverage for find candidates",
                        default=10)
    parser.add_argument('--min_allele_freq', type=float, help="Minimum allele frequency for find candidates",
                        default=0.125)
    parser.add_argument('--min_mq', type=int, help="minimum mapping quality required",
                        default=10)
    parser.add_argument('--min_bq', type=int, help='minimum base quality required',
                        default=10)
    parser.add_argument('--vcf_fn', type=str, help='candidate sites vcf file')
    parser.add_argument('--ctg_name', type=str, help='the name of suquence name, e.g., chr1',
                        required=True)
    parser.add_argument('--ctg_start', type=int, help="start position of sequence",
                        required=True)
