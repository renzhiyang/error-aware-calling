...
def samtools_view_from(ctg_name, ctg_start, ctg_end, bam_fn, min_mq, samtools):
    ...
    subprocess = Popen(shlex.split(f"{samtools} view -q {min_mq} {bam_fn} {region}"), stdout=PIPE)
    return subprocess
...
def create_tensor(args):
    ...
    for line in candidates:
        ...
        subprocess = samtools_view_from(ctg_name, candidate_pos, candidate_pos + 1, bam_fn, min_mq, samtools)
        ...
        for i, read in enumerate(reads):
            tensor_sequence = get_tensor_sequence_from(read, candidate_pos, window_width)
...
def main():
    parser = argparse.ArgumentParser(description='Generate tensor around candidates from alignment file for trained model')
    ...
    parser.add_argument('--samtools', type=str, help="Path to the samtools", 
                        default="samtools")
    parser.add_argument('--candidates_fn', type=str, 
                        help="Path to candidate sites generated by find_candidates.py")
    parser.add_argument('--tensor_fn', type=str,
                        help="Tensor output, will be used to call variants")
    parser.add_argument('--min_mq', type=int, default=20,
                        help="Minimum mapping quality of a read")
    parser.add_argument('--tensor_window_width', type=int, default=99,
